...
export default function ServiceCard({ title, description, icon, features }: ServiceCardProps) {
  return (
    <div className="bg-white rounded-2xl p-8 shadow-lg hover:shadow-xl transition-shadow duration-300 border border-gray-100">
      <div className="w-14 h-14 bg-[#1a6530]/10 rounded-xl flex items-center justify-center text-[#1a6530] mb-6">
        {icon}
      </div>
      
      <h3 className="text-xl font-bold text-[#143d24] mb-3">{title}</h3>
      ...
      <Link 
        to="/contact" 
        className="inline-flex items-center text-[#143d24] font-semibold hover:text-[#1a6530] transition-colors"
      >
        Demander un devis
